...
import {
  EVENTS_GC,
  EVENTS_STALE,
  SUBSCRIBED_EVENTS_GC,
  SUBSCRIBED_EVENTS_STALE,
} from '@/api/queryConfig';
...
export function useCalendarData(date: Date) {
  ...
  const { data: subscribedEvents = [] } = useQuery<CalendarEvent[]>({
    queryKey: [activeAccountId, 'subscribed-events', subscribedKeys, start.toISOString(), end.toISOString()],
    queryFn: () =>
      activeAccount
